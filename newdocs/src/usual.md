...
1. **Clearly identify what you plan to report**.
    ...
    **If you are reporting a problem**
    ...
    - A minimal example that illustrates the problem, including only relevant portions
      of your code
    - If configuration files are involved, the portions of those configuration files
      that relate to the `a_p` function being invoked.
        - Ensure that any secrets (passwords or keys) are replaced with something
          like *SECRET*.
    - A consicse description of the actual behavior and a similar description of
...
4. **Check that the issue is not already present in our backlog.**
   ...
   Go to our
   [Issues List](https://github.com/application_properties/pymarkdown/issues),
   remove
