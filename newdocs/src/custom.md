...
## custom configuration data loaders

## custom configuration data accessors

## Required Properties And Validation Functions

## Strict Mode and type conversion when getting property values

## Setting manual properties

## Layering

## custom fred

not start wiht ., end with . or have ..
cannot be empty, contain a '.' or '=', \n, \t

ApplicationProperties(allow_separator_in_keys)

## Getters and Null

**Important Note on `None` and `null`:**
In Python, `None` is a valid value. In YAML/JSON, `null` becomes `None` in Python.

- If the key is **missing** from the config file: `default_value` is used.
- If the key is present but set to `null` (e.g., `log-level: null` in YAML): The
  getter returns `None`. If you have set `default_value="INFO"`, this **default
  will NOT be used** because the key *exists*.

To treat `null` values the same as missing keys, use `is_required=False` (default)
and check for `None` in your code, or use `default_value` only if you are certain
the key will be absent, not null. This distinction allows you to differentiate between
'value not configured' (missing) and 'value explicitly unset' (null). If a null
value is unacceptable, use the `is_required=True` parameter below to raise an error
explicitly.
...
to [work by default](./basic-concepts.md#our-philosophy-of-work-by-default).
...
if no configuration item is found.  To keep things simple [until later](./custom.md#strict-mode-and-type-conversion-when-getting-property-values),
...
[hierarchical](./basic-concepts.md#hierarchical) key of `some.thing`.
...
`get_integer_property` with the [flattened hierarchical](./basic-concepts.md#flattened-hierarchical)
...
stored [hierarchically](./basic-concepts.md#hierarchical) is important, allowing
...
An important concept to note is the concept of [configuration layering](./layering.md).
...
[next section](./custom.md#required-properties-and-validation-functions) ).
...
[work by default](./basic-concepts.md#our-philosophy-of-work-by-default) philosophy
